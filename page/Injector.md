...
Among other things, Qubus Injector recursively instantiates class dependencies based on the parameter
...
each reflection offers the opportunity to cache the results if you're worried about speed. Qubus Injector
caches any reflections it generates to minimize the potential performance impact.

> Qubus Injector **is NOT** a Service Locator. DO NOT turn it into one by passing the injector into your
...
To start using the injector, simply create a new instance of the `Qubus\Injector` ("the Injector")
class:

```php
$injector = new Qubus\Injector;
```
...
You may also specify injection definitions at call-time with `Qubus\Injector::make`. Consider:
...
specified class, but only in the context of that particular call to `Qubus\Injector::make`.
...
NOT class names. If the colons had been omitted above, Qubus Injector would attempt to instantiate classes of
...
parameter name is prefixed with a `:`, Qubus Injector will inject the value directly without attempting to
instantiate it.
...
specify definitions for this sort of thing everywhere it might be used in the app. Qubus Injector mitigates
...
and should generally be avoided. `Qubus\Injector` makes sharing class instances across contexts a
...
wiring together your application using Qubus Injector. Here, we want to inject a single database connection
...
> By sharing an instance of a class, `Qubus\Injector` will always use that instance when
...
`Qubus\Injector::make` will have no effect. Once shared, an instance will always be returned for
instantiations of its type until the object is un-shared or refreshed:

## Instantiation Delegates

Often factory classes/methods are used to prepare an object for use after instantiation. Qubus Injector
...
`Qubus\Injector::delegate`. Additionally you may specify the name of a delegate class that
...
additional post-instantiation mutations. Qubus Injector accommodates these use cases with its
...
In addition to provisioning class instances using constructors, Qubus Injector can also recursively instantiate
...
`Qubus\Injector` resolves dependencies in the following order:
...
3. If a call-time definition is passed to `Qubus\Injector::make`, that definition will be used
...
### Qubus Injector Is NOT A Service Locator!
...
> **IMPORTANT:** do not use Qubus Injector like a Service Locator!
...
see how we can use Qubus Injector to inject the same `PDO` instance across the entire scope of our application.
...
In the above example the Qubus Injector DIC allows us to write fully testable, fully OO controllers that ask
...
eliminate evil Singletons using the sharing capabilities of the Qubus Injector DIC. In the front controller
code, we share the request object so that any classes instantiated by the `Qubus\Injector` that ask
